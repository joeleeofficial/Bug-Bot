...
let addNote = {
  ...
  execute: function(bot, channelID, userTag, userID, command, msg, trello, db) {
    if(channelID === config.channels.queueChannel) {
      ...
      if(isNotMod === -1) {
        utils.botReply(bot, userID, channelID, "only Trello Mods and above can add notes to queue reports.", command, msg.id);
        return;
      }
    }
    ...
    db.get("SELECT reportStatus, reportMsgID, trelloURL FROM reports WHERE id = ? OR trelloURL = ?", [key, key], function(error, reportInfo) {
      if(!!error) {
